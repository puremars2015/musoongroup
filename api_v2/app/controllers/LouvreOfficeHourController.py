...
router = APIRouter(
    prefix="/louvre/office_hour",
    tags=["louvre"],
)

table_name = "louvre_office_hour"
...
async def post(requestBody: List[Dict]):
    db = DatabaseHelper('app/database/musoon.db')
    try:
        for rowData in requestBody:
            db.insert(table_name, rowData)
...
# 依據日期搜尋數筆資料
@router.post("/search_week")
async def post(requestBody: List[Any]):
    db = DatabaseHelper('app/database/musoon.db')
    r = db.selectByDays(table_name, ["*"], requestBody)
    return r
...
# 更新數筆資料
@router.put("/update_week")
async def put(requestBody: List[Dict]):
    db = DatabaseHelper('app/database/musoon.db')
    try:
        for rowData in requestBody:
                db.update(table_name, rowData, { "id": rowData["id"] })
    except Exception as e:
        return { "message": "error", "error": str(e) }
