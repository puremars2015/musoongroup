...
export class FrontPageCarouselEditComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.id != 'add') {
      let obj = await firstValueFrom(this.service.ReadById('musoon','front_page',params['id']));
      this.data = obj[0];
    }
  }
  ...
  async save() {
    ...
    if (self.id === 'add') {
      // 新增
      let r = await firstValueFrom(self.service.UploadFile('musoon','front_page/upload',self.selectedFile));
      if (r['message']!="success") {
        alert(r['message']);
        return;
      }
      self.data['content'] = r['path'];
      let r2 = await firstValueFrom(self.service.Create('musoon','front_page',self.data));
      alert(r2['message']);
    }
    else {
      // 修改
      ...
      let r = await firstValueFrom(self.service.Update('musoon','front_page',self.data));
      alert(r['message']);
    }
  }
