...
export class FrontPageCarouselComponent implements OnInit {
  ...
  async create() {
    let r = await firstValueFrom(this._service.Create('musoon','front_page',this.data));
    console.log(r);
  }

  async query() {
    this.articles = await firstValueFrom(this._service.Read('musoon','front_page'));
  }

  move(i: string) {
    // 導航到 meet-us-edit 頁面, 並同時帶上文章的 index
    this.router.navigate(['/musoon/frontpage-edit'], { queryParams: { id: i } });
  }
  ...
  async remove(i: number) {
    if (confirm('確定要刪除嗎?')) {
      ...
      // 刪除
      let r = await firstValueFrom(this._service.Remove('musoon','front_page',i));
      alert(r['message']);
