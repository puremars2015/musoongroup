...
export class MeetUsEditComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    let obj = await firstValueFrom(self.service.Read('louvre','meet_us'));
    if (obj && obj.length > 0) {
      self.data = obj[0];
      self.id = '1';
    }
  ...
  async save() {
    ...
    if (self.id === 'add') {
      // 新增
      let r = await firstValueFrom(self.service.UploadFile('louvre','meet_us/upload',self.selectedFile));
      if (r['message']!="success") {
        alert(r['message']);
        return;
      }
      self.data['content'] = r['path'];
      let r2 = await firstValueFrom(self.service.Create('louvre','meet_us',self.data));
      alert(r2['message']);
    }
    else {
      // 修改
      ...
      let r = await firstValueFrom(self.service.Update('louvre','meet_us',self.data));
      alert(r['message']);
    }
  }
