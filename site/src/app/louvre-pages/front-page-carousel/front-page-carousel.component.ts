...
export class FrontPageCarouselComponent implements OnInit {
  ...
  async create() {
    let r = await firstValueFrom(this._service.Create('louvre','front_page',this.data));
    console.log(r);
  }

  async query() {
    this.articles = await firstValueFrom(this._service.Read('louvre','front_page'));
  }

  move(i: string) {
    // 導航到 meet-us-edit 頁面, 並同時帶上文章的 index
    // window.location.href = `/backstage/musoon/frontpage-edit?id=${i}`;
    this.router.navigate(['/louvre/frontpage-edit'], { queryParams: { id: i } });
  }
  ...
  async remove(i: number) {
    if (confirm('確定要刪除嗎?')) {
      ...
      // 刪除
      let r = await firstValueFrom(this._service.Remove('louvre','front_page',i));
      alert(r['message']);
