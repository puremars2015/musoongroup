...
export class OurNewsEditComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.id != 'add') {
      let obj = await firstValueFrom(this.service.ReadById('louvre','our_news',params['id']));
      this.data = obj[0];
    }
  }
  ...
  async save() {
    ...
    if (self.id === 'add') {
      // 新增
      let r = await firstValueFrom(self.service.UploadFile('louvre','our_news/upload',self.selectedFile));
      if (r['message']!="success") {
        alert(r['message']);
        return;
      }
      self.data['img_url'] = r['path'];
      let r2 = await firstValueFrom(self.service.Create('louvre','our_news',self.data));
      alert(r2['message']);
    }
    else {
      // 修改
      ...
      let r = await firstValueFrom(self.service.Update('louvre','our_news',self.data));
      alert(r['message']);
    }
  }
