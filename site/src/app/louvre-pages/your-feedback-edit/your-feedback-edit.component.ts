...
export class YourFeedbackEditComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.id != 'add') {
      let obj = await firstValueFrom(this.service.ReadById('louvre','your_feedback',params['id']));
      this.data = obj[0];
    }
  }
  ...
  async save() {
    ...
    if (self.id === 'add') {
      // 新增
      let r = await firstValueFrom(self.service.UploadFile('louvre','your_feedback/upload',self.selectedFile));
      if (r['message']!="success") {
        alert(r['message']);
        return;
      }
      self.data['img_url'] = r['path'];
      let r2 = await firstValueFrom(self.service.Create('louvre','your_feedback',self.data));
      alert(r2['message']);
    }
    else {
      // 修改
      ...
      let r = await firstValueFrom(self.service.Update('louvre','your_feedback',self.data));
      alert(r['message']);
    }
  }
  ...
  async UploadOnClick() {
    let self = this;
    if (self.newPicture) {
      let r = await firstValueFrom(self.service.UploadFile('louvre','your_feedback/upload',self.newPicture));
